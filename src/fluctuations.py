...
def evolve_fluctuations(S: np.ndarray, J:np.ndarray,  τ:np.double, β: np.ndarray, γ: np.ndarray):
    ...
    n_ij_pm = lambda k, idx, pm: solution(S[-1][:, 2])[idx] * S[k][idx, 0] + pm * 1j * S[k][idx, 1]    
    ...
    for k in range(γ.shape[0]):    
        ...
        for i in range(N - 1):
            ...
            for j in range(i + 1, N - 1):
                A[k][i][j] = -γ[k] * J[i][j] * n_ij_pm(k, i, 1) * n_ij_pm(k, j, -1)
                B[k][i][j] = -γ[k] * J[i][j] * n_ij_pm(k, i, 1) * n_ij_pm(k, j,  1)
